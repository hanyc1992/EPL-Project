...
public:
	...
	template <typename T2>
	valarray(const valarray<T2>& that) : vector<T>(that.size()) {
		for (uint64_t i = 0; i < that.size(); ++i)
			this->operator[](i) = that[i];
	}

	template <typename OP, typename LHS, typename RHS>
	valarray(const BinaryOp<OP, LHS, RHS>& that) : vector<T>(that.size()) {
		for (uint64_t i = 0; i < that.size(); ++i)
			this->operator[](i) = that[i];
	}

	template <typename OP, typename T2>
	valarray(const UnaryOp<OP, T2>& that) : vector<T>(that.size()) {
		for (uint64_t i = 0; i < that.size(); ++i)
			this->operator[](i) = that[i];
	}
	/*****************************************************************************/
...
/*********************************assignment operator****************************************/
	template <typename T2>
	valarray<T> operator=(const valarray<T2>& that) {
	...
	//template <typename T2>
	//valarray<T> operator=(const T2& that) {
	//	uint64_t size_old = this->size();
	//	uint64_t size_new = std::min<uint64_t>(size_old, that.size());
	//	for (uint64_t i = size_new; i < size_old; ++i)
	//		this->pop_back();
...
template <typename T>
std::ostream& operator<<(std::ostream& out, valarray<T> val) {
...
template <typename OP, typename LHS, typename RHS>
std::ostream& operator<<(std::ostream& out, BinaryOp<OP, LHS, RHS> bi) {
	for (auto const& p : bi)
		out << p << " ";
	out << std::endl;
	return out;
...
template <typename OP, typename T>
std::ostream& operator<<(std::ostream& out, UnaryOp<OP, T> un) {
	for (auto const& p : un)
		out << p << " ";
	out << std::endl;
	return out;
